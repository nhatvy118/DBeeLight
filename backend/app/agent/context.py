...
class DbContext:
    ...
    def adapter_for_table(self, table_name: str) -> Optional["DatabaseAdapter"]:
        """Session tables always have the t_ prefix (generated by file ingestion)."""
        if self.primary is not None and self.session is not None:
            return self.session if str(table_name).startswith("t_") else self.primary

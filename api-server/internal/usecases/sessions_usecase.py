...
class SessionsUseCase:
    ...
    async def list_sessions(self, user_key: str):
        agent = await self._agent_repo.get_agent(user_key=user_key)
        return agent.session_manager.list_sessions() if agent.session_manager else []

    async def create_session(self, user_key: str, name: str | None):
        agent = await self._agent_repo.get_agent(user_key=user_key)
        session_id = agent.session_manager.create_session(name) if agent.session_manager else None
